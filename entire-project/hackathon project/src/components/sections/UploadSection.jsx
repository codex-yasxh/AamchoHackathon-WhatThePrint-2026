...
function UploadSection({ files, totalPages, pageRate, onProcessFiles, onUpdateCopies, onRemoveFile, onClearFiles }) {
  const onDrop = (event) => {
      ...
      {files.length > 0 && (
        <div className="mt-6 overflow-x-auto rounded-[10px] border border-[var(--border)]">
          <table className="w-full min-w-[700px] text-xs">
            ...
            <tbody>
              {files.map((file, index) => (
                <tr key={file.id} className="border-b border-[var(--line)] bg-[var(--surface)]">
                  ...
                  <td className="p-2 text-center">
                    <button className="text-[var(--muted)] transition hover:text-[var(--danger)]" type="button" onClick={() => onRemoveFile(file.id)}>
                      x
                    </button>
                  </td>
                </tr>
          ...
          <div className="flex items-center justify-between bg-[var(--surface-strong)] px-3 py-2 text-[11px] text-[var(--muted)]">
            <span>
              <strong className="text-[var(--text)]">{files.length}</strong> files · <strong className="text-[var(--text)]">{totalPages}</strong> total print pages
            </span>
