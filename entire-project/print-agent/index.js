...
async function processJob(job) {
  ...
  try {
    ...
    tempFilePath = getTempOutputPath(job.id, job.file_url);
    logInfo(`Processing job ${job.id}`, { tempFilePath });

    await downloadToTempFile(job.file_url, tempFilePath);

    const printOptions = PRINTER_NAME ? { printer: PRINTER_NAME } : undefined;
    await print(tempFilePath, printOptions);

    await markJobStatus(job.id, 'DONE');
    logInfo(`Print success for job ${job.id} (DONE)`);
  } catch (error) {
